  ...
  if (MODE === "preflight") {
    ...
    try {
      ...
      if (Number(current.revision_number) === 2 && nearlyEqual(current.flight_time_hours, TARGET.correctedFlightTimeHours)) {
        ...
        await client.query(`INSERT INTO audit.audit_events (request_id, actor_type, actor_user_id, operation_source, aircraft_id, entity_type, entity_id, entity_key, action_code, before_state, after_state, reason, metadata, payload_version) VALUES ($1::uuid, 'SYSTEM', NULL, 'MIGRATION', $2::uuid, 'FLIGHT_RECORD', $3::uuid, jsonb_build_object('legacy_id', $4::text, 'flight_date', $5::text, 'registration', $6::text), 'SOURCE_RECONCILED', jsonb_build_object('current_revision_id', $7::uuid, 'revision_number', 1, 'flight_time_hours', $8::numeric, 'time_in_service_hours', $9::numeric), jsonb_build_object('current_revision_id', $10::uuid, 'revision_number', 2, 'flight_time_hours', $11::numeric, 'time_in_service_hours', $9::numeric), $12::text, jsonb_build_object('source_spreadsheet_id', $13::text, 'historial_aeronave_row', $14::integer, 'computacion_horas_row', $15::integer, 'source_confirmation', 'Historial Aeronave corrected to 2.1; Computacion Horas independently confirms 2.1', 'legacy_id', $4::text), 1)`, [requestId, current.aircraft_id, flightId, TARGET.legacyId, TARGET.flightDate, TARGET.aircraftRegistration, oldRevisionId, TARGET.oldFlightTimeHours, TARGET.timeInServiceHours, newRevisionId, TARGET.correctedFlightTimeHours, TARGET.correctionReason, TARGET.spreadsheetId, TARGET.historySourceRow, TARGET.computationSourceRow]);
